      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {filteredProducts.map((product) => (
          <motion.div key={product.id} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <div className="p-6 border  shadow-xl bg-white text-center hover:shadow-2xl transition-all">
              ...
              <p className="text-2xl font-bold text-red-600 mb-4">₹{product.price}</p> {/* ₹ symbol added */}
              <button className="w-full bg-red-500 text-white p-3 rounded-lg font-semibold hover:bg-blue-600 transition-all transform hover:scale-105">
                Buy Now
              </button>
            </div>
          </motion.div>
